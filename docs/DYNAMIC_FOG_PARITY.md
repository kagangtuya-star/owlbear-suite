...
dynfog/
  ...
  geom/
    ...
    cardinal.ts                Cardinal 样条 → 折线
    drawing.ts                 Drawing 类型 + drawingToPolylines()
    polyline.ts                弧长寻址 / 取子段 / 按开口切分
  reconcile/
  ...
  light/
    ...
    createLightMenu.ts         添加光源 / 光源设置 / 移除光源
  overlay.ts                   跟随迷雾工具显隐的 GM 指示层
  index.ts                     setup / teardown
```

原 `fullFog/door/`、`fullFog/light/index.ts`、`fullFog/index.ts::syncLocalWalls()`
...
是**每个客户端**都要加载的，当前全部 JS 加起来才 ~400 KB，dist 总共 12 MB（绝大
部分是骰子模型等静态资源）。为一个子模块给所有玩家加 6.8 MB 的 WASM，代价过高。

因此几何内核改为纯 TS：

| Skia 能力 | 本实现 |
| --- | --- |
| 曲线采样 | `geom/drawing.ts` 逐 command 采样（QUAD/CUBIC 解析式，复用 `output/samplePath.ts` 的思路）；Curve 走 `geom/cardinal.ts` |
| 弧长寻址 | `geom/polyline.ts` 的归一化弧长参数 `t ∈ [0,1]`（`polyIndex` + `t`） |
| 布尔差集挖门 | `geom/polyline.ts::splitPolylineByOpenings()` — 在 t 域上切分折线 |
| `stroke()` 取轮廓 | **不做**：墙直接取图形的中心线轮廓 |

### 3.1 唯一一处刻意的行为差异：中心线 vs 描边轮廓
...
其他所有行为（哪些 item 变墙、门的开合语义、光源参数与表现、指示器颜色/图标、
工具的拖拽手感、快捷键）都与上游一致。
...
存在**被画的那个 FOG Drawing** 的 metadata 上：

```ts
// key: `${PLUGIN_ID}/openings`
interface Opening {
...
- **窗**：默认 `open: true`（透光 = 不挡视线），蓝色；关闭（拉上百叶）后灰蓝并挡视线。

墙的推导规则对两者统一：`open === true` ⇒ 该段从墙里挖掉。

> OBR 的 `Wall` 只影响**视线**，不影响**移动**，所以「窗户能看不能走」在 OBR 里
> 无法用墙表达。这里把窗定义为「默认常开、可关闭、视觉上与门区分」的开口。

**上游兼容**：`opening/read.ts` 同时接受上游 `rodeo.owlbear.dynamic-fog/doors`
形状（`{open, start:{index,distance}, end:{...}}`），按折线长度换算成 `t`。只读，
不回写。
...
OBR 里 FOG 图层的 item 玩家**读得到**（客户端要自己渲染迷雾），但**写不了**。
所以：

```
玩家点击门指示器
  → OBR.broadcast.sendMessage("com.obr-suite/fullFog/toggle", {itemId, openingId}, {destination:"REMOTE"})
  → GM 的 background 收到 → 校验开关权限 → OBR.scene.items.updateItems 改 open
  → 全客户端 items.onChange → Reconciler → 墙重算 + 指示器换色
```

玩家侧的点击入口有两个（互为兜底）：

1. **「开关门窗」工具**（`OBR.tool.create`）——工具栏上一个独立按钮，所有客户端
   都注册。选中后点门即切换。这是保证可用的主路径。
2. **选中即切换**——指示器 handle 是可选中的本地 item，`OBR.player.onChange` 检测
   到选中了 handle 就立即取消选中并触发切换。不依赖切工具，但如果 OBR 未来改了
   本地 item 的可选中性，退化到路径 1。
...
开关权限由场景设置 `fogPlayerDoors` 控制（默认开）。

## 6. 设置项（新增）

| key | 默认 | 说明 |
| --- | --- | --- |
| `fogPlayerDoors` | `true` | 玩家可见门窗指示器并可自行开关 |
| `fogDoorOverlayAlways` | `false` | GM 不选迷雾工具时也常显门窗指示器 |

## 7. 实施顺序

1. 几何内核 `geom/`
2. Reconciler 框架
3. Actors / Reactors（墙 → 开口 → 光源 → 指示层）
4. 工具（Line / 门 / 窗 / 玩家开关）
5. 光源菜单 + 设置面板 UI
6. 接线：`fullFog/index.ts`、`state.ts`、`settings.ts`、`background.ts`
7. 构建 / 类型检查 / 验证
8. 输出 dev ↔ stable 差异清单

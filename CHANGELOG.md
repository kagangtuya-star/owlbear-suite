...
- **Darkvision renders the colour disc it was supposed to.** The greyscale ring was on the `POST_PROCESS` layer, which Owlbear runs over the finished frame in VIEWPORT space: the world-space box the shader is built around was discarded, `coord` spanned the screen instead of the light, the centre landed off-screen, and every pixel evaluated as past the colour radius — so switching darkvision on drained the entire view with no colour anywhere. Moved to `ATTACHMENT`, which is world-space and sits above MAP and CHARACTER. Sitting below fog is correct, since desaturated black is still black. Drawn at the bottom of the layer so HP bars and buff bubbles keep their colour.
